  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="flex items-start justify-between">
        <div>
          <div className="text-page-title font-semibold text-text-primary">Inventory</div>
          <div className="mt-1 text-secondary text-text-muted">Manage products, stock and warehouses.</div>
        </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <button
          ...
          className={
            "rounded-input border px-3 py-2 text-body transition-colors duration-fast " +
            (lowStockOnly
      ...
      <Card className="overflow-hidden">
        {loading ? (
          ...
          <div className="p-8 text-center">
            <div className="text-body text-text-primary">Unable to load inventory.</div>
          ...
          <EmptyState hasFilters={search !== "" || lowStockOnly} />
        ) : (
          <>
            <table className="w-full text-body">
              ...
              <tbody>
                {items.map((p) => (
                  <tr key={p.id} className="border-b border-border last:border-0 hover:bg-surface-secondary">
                    <td className="px-4 py-2.5 text-text-primary">{p.name}</td>
                    <td className="px-4 py-2.5 text-text-muted">{p.sku}</td>
                    <td className="px-4 py-2.5 text-text-secondary">{p.categoryName}</td>
                    <td className="px-4 py-2.5 text-right tabular-nums text-text-primary">
                      {p.totalOnHand} {p.unitAbbreviation}
                    </td>
  ...
  return (
    <div className="p-10 text-center">
      ...
      <div className="mt-1 text-secondary text-text-muted">
        {hasFilters ? "Try a different search term or clear the filter." : "Add your first product to start managing inventory."}
      </div>
    </div>
  );
}
