...
import { AlertCircle, Box, CheckCircle2, Filter, Layers, Package, Plus, Sparkles } from "lucide-react";
...
export default function InventoryPage() {
  ...
  const estimatedCatalogValue = items.reduce(
    (acc, p) => acc + p.sellingPrice * p.totalOnHand,
    0
  );
  ...
  return (
    <div className="flex flex-col gap-6 sm:gap-7 pb-8">
      {/* Header */}
      <PageHeader
        breadcrumbs={["Inventory", "Catalog"]}
        title="Inventory"
        description="Monitor stock levels, warehouse products and automated replenishment thresholds."
        actions={
          <Button
            variant="primary"
      ...
      <section className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <KpiCard
          ...
          variant="dark"
          tag="Live Stock"
          changeLabel="100% synchronized"
          changeDirection="up"
        />
        <KpiCard
          label="Healthy Stock"
          value={healthyCount.toLocaleString()}
          detail={`${healthyCount} items above reorder target`}
        ...
        <KpiCard
          ...
          detail={
            lowStockCount > 0
              ...
              : "Zero shortages reported"
          }
          changeLabel={lowStockCount > 0 ? "Requires restock" : "Optimal"}
          changeDirection={lowStockCount > 0 ? "down" : "up"}
        />
        <KpiCard
          label="Estimated Stock Value"
          value={<MoneyDisplay amount={estimatedCatalogValue} />}
          detail="Based on active on-hand units"
          changeLabel="+4.2% catalog growth"
          changeDirection="up"
        />
      </section>
      ...
      <div className="overflow-hidden rounded-[18px] border border-[#E3E9E5] bg-white shadow-sm">
        ...
        <div className="flex flex-col gap-4 border-b border-[#EAEFEA] p-4 sm:p-5 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <div className="flex items-center gap-2">
              <h2 className="text-sm font-bold text-[#142019]">Stock Registry</h2>
              <span className="rounded-full bg-[#EBF4EE] px-2 py-0.5 text-[10px] font-bold text-[#1F7A4D]">
                {items.length} items on page
              </span>
            </div>
            <p className="mt-0.5 text-[12px] text-[#697B70]">
              Filter products, monitor SKU thresholds, and check on-hand balances.
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2.5">
            ...
            <button
              ...
              className={`inline-flex items-center gap-1.5 rounded-xl border px-3.5 py-2 text-xs font-semibold transition-all ${
                lowStockOnly
                  ? "border-[#E8B4B4] bg-[#FDF2F2] text-[#C84A4A]"
                  : "border-[#D7DFDA] bg-white text-[#4A5E51] hover:bg-[#F2F5F3]"
              }`}
            >
              <AlertCircle size={14} />
        ...
        {loading ? (
          ...
          <EmptyState
            ...
            description={
              lowStockOnly
                ? "All catalog items currently satisfy their replenishment reorder thresholds."
                : search
                ? `No products matched "${search}". Try checking the spelling or resetting filters.`
                : "Add your first product to begin tracking warehouse stock levels and order fulfillments."
            }
            actionLabel={
              lowStockOnly
                ? "Clear Filter"
            ...
            <div className="overflow-x-auto">
              <table className="w-full min-w-[840px] text-xs">
                ...
                <tbody className="divide-y divide-[#EAEFEA]">
                  {items.map((p) => {
                    const isAlert = p.isLowStock || p.totalOnHand <= 0;
                    return (
                      <tr
                        key={p.id}
                      ...
                      >
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-3">
                            <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#EDF5F0] text-[#1F7A4D]">
                              <Box size={17} />
                            </div>
                            <div>
                              <div className="font-bold text-[#142019]">{p.name}</div>
                              <div className="text-[10px] text-[#7A8C81]">
                                Measured in {p.unitAbbreviation}
                              </div>
                            </div>
                          </div>
                        </td>
                        <td className="px-6 py-4">
                          <span className="font-mono font-semibold text-[#485C50] bg-[#F0F4F1] px-2 py-0.5 rounded-md text-[11px]">
                            {p.sku}
                          </span>
                        </td>
                        <td className="px-6 py-4 font-medium text-[#485C50]">
                          {p.categoryName}
                        </td>
                        <td className="px-6 py-4 text-right">
                          <span className="font-bold text-[#142019] tabular-nums">
                            {p.totalOnHand}
                          </span>{" "}
                          <span className="text-[11px] text-[#7A8C81]">
                            {p.unitAbbreviation}
                          </span>
                        </td>
                        <td className="px-6 py-4 text-right text-[#6E8075] tabular-nums">
                          {p.reorderLevel} {p.unitAbbreviation}
                        </td>
