...
import { getMe, getNotifications, MeResult } from "@/lib/nexora-api";
...
export default function ShellLayout({ children }: { children: React.ReactNode }) {
  const router = useRouter(); const pathname = usePathname(); const [me, setMe] = useState<MeResult | null>(null); const [checked, setChecked] = useState(false); const [menuOpen, setMenuOpen] = useState(false); const [unreadCount, setUnreadCount] = useState(0);
  useEffect(() => { setMenuOpen(false); }, [pathname]);
  useEffect(() => {
    if (!me) return;
    getNotifications({ unreadOnly: true, page: 1, pageSize: 1 }).then((result) => setUnreadCount(result.totalCount)).catch(() => {});
  }, [me, pathname]);
  ...
  return (
    <div className="min-h-screen bg-[#F5F7F5] lg:flex">
      <Sidebar permissions={me.permissions} open={menuOpen} onClose={() => setMenuOpen(false)} userEmail={me.email} unreadCount={unreadCount} />
      <div className="min-w-0 flex-1">
        <Topbar breadcrumb={BREADCRUMBS[pathname ?? ""] ?? "NEXORA"} userEmail={me.email} unreadCount={unreadCount} onMenuClick={() => setMenuOpen(true)} />
