    ...
    <main className="relative min-h-screen overflow-hidden bg-[#071C14]" style={{ fontFamily: "var(--font-family-primary)" }}>
      <div className="pointer-events-none fixed inset-0 z-0">
        <AcidSquares
          ...
          mouseInteraction
        />
        <div className="absolute inset-0 bg-[#071C14]/35" />
        <div className="absolute inset-0 bg-gradient-to-br from-[#071C14]/70 via-transparent to-[#0A3B27]/65" />
      </div>

      <div className="relative z-10 flex min-h-screen w-full flex-col lg:flex-row">
        <section className="flex min-h-[48vh] flex-1 flex-col justify-between px-7 py-8 sm:px-12 sm:py-10 lg:min-h-screen lg:px-16 lg:py-12 xl:px-20">
          <div className="flex items-center gap-3">
            <div className="flex h-11 w-11 items-center justify-center rounded-xl border border-white/15 bg-white/10 text-sm font-bold text-white backdrop-blur-md">NX</div>
            <div>
              <div className="text-xl font-bold tracking-tight text-white">NEXORA</div>
              <div className="text-[10px] uppercase tracking-[0.24em] text-[#A8C8B6]">Business workspace</div>
            </div>
          </div>

          <div className="max-w-xl py-12 lg:py-0">
            <p className="mb-5 text-xs font-semibold uppercase tracking-[0.24em] text-[#9DDBB5]">Operations, connected</p>
            <h1 className="max-w-lg text-4xl font-semibold leading-[1.03] tracking-[-0.045em] text-white sm:text-5xl xl:text-6xl">Everything your business needs, in one place.</h1>
            <p className="mt-6 max-w-md text-sm leading-6 text-[#C2D8CB]">Keep sales, stock, purchasing and finance moving together without adding another layer of complexity.</p>
            <div className="mt-8 flex flex-wrap gap-2">
              {MODULES.map((module) => <span key={module} className="rounded-full border border-white/15 bg-white/10 px-3.5 py-1.5 text-[11px] font-medium text-white/90 backdrop-blur-sm">{module}</span>)}
            </div>
          </div>
        ...
        <section className="flex min-h-[52vh] w-full items-center justify-center bg-white/95 px-6 py-10 shadow-[-20px_0_70px_rgba(0,0,0,0.08)] backdrop-blur-xl sm:px-12 lg:min-h-screen lg:w-[43%] lg:px-14 xl:w-[40%] xl:px-16">
          <div className="w-full max-w-[390px]">
            <div className="mb-8 lg:hidden"><div className="text-xl font-bold tracking-tight text-[#123B2A]">NEXORA</div><div className="mt-1 text-[10px] uppercase tracking-[0.22em] text-[#718278]">Business workspace</div></div>
            ...
            <div className="mt-7 border-t border-[#E1E7E3] pt-5 text-[11px] leading-5 text-[#78877F]">Demo mode activates automatically when the workspace API is unavailable.</div>
          </div>
        </section>
      </div>
    </main>
