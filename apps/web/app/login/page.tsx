...
export default function LoginPage() {
  ...
  return (
    <main
      className="relative flex h-[100dvh] min-h-[100svh] w-full overflow-hidden bg-[#071C14]"
      style={{ fontFamily: "var(--font-family-primary)" }}
    >
      <div className="pointer-events-none fixed inset-0 z-0 h-[100dvh] min-h-[100svh] w-full overflow-hidden">
        <AcidSquares
          color1="#071C14"
          color2="#185F3D"
          color3="#B9EACB"
      ...
      <div className="relative z-10 flex h-full w-full items-center justify-center overflow-y-auto px-4 py-5 sm:px-6 sm:py-7 lg:px-8">
        <div className="w-full max-w-[470px]">
          ...
          <section className="rounded-[24px] border border-white/70 bg-white/[0.97] p-6 shadow-[0_30px_100px_rgba(0,0,0,0.32)] backdrop-blur-2xl sm:p-9">
            <div className="mb-7">
              <div className="mb-3 flex items-center gap-2 text-[10px] font-semibold uppercase tracking-[0.2em] text-[#1F7A4D]">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div>
                <div className="mb-2 flex items-center justify-between">
                  <label htmlFor="login-password" className="block text-xs font-medium text-[#405248]">Password</label>
                  <span className="text-xs text-[#8A978F]">Forgot password?</span>
                </div>
                <div className="relative">
                  ...
                  <button type="button" onClick={() => setShowPassword((value) => !value)} className="absolute right-3 top-1/2 -translate-y-1/2 text-[#87968C]" aria-label={showPassword ? "Hide password" : "Show password"}>
                    {showPassword ? <EyeOff size={17} /> : <Eye size={17} />}
                  </button>
                </div>
              </div>
              {error && <div className="rounded-xl border border-[#F1C5C5] bg-[#FFF7F7] px-4 py-3 text-xs font-medium text-[#BE4A4A]">{error}</div>}
              <button id="login-submit" type="submit" disabled={loading} className="flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-[#123B2A] text-sm font-semibold text-white shadow-[0_10px_24px_rgba(18,59,42,0.18)] transition hover:bg-[#184E37] active:scale-[0.99] disabled:opacity-60">
                {loading ? "Signing in..." : <>Continue to workspace <ArrowRight size={15} /></>}
              </button>
            </form>
          ...
          <div className="mt-4 flex flex-wrap items-center justify-center gap-x-4 gap-y-2 text-[10px] font-medium uppercase tracking-[0.12em] text-white/55">
            {MODULES.map((module) => <span key={module}>{module}</span>)}
          </div>
          <div className="mt-3 text-center text-[9px] font-medium uppercase tracking-[0.18em] text-white/35">
